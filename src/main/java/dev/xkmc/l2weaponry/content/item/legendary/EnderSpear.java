...
public class EnderSpear extends SpearItem implements LegendaryWeapon {
	...
	public InteractionResultHolder<ItemStack> use(Level level, Player player, InteractionHand hand) {
		...
		var hit = RayTraceUtil.rayTraceEntity(player, 64, e -> e instanceof LivingEntity);//TODO config
		if (hit != null && teleport(player, hit.getLocation()))
	...
	private boolean teleport(Player player, Vec3 target) {
	...
	public void appendHoverText(ItemStack pStack, @Nullable Level pLevel, List<Component> list, TooltipFlag pIsAdvanced) {
		list.add(LangData.ENDER_SPEAR.get());
	}
